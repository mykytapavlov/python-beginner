...
if user_input_genre == 'y':
    ...
    while enter_genre_input not in GENRES.keys():
        print('Genre', enter_genre_input, 'not found. Please try again.')
        enter_genre_input = input('Enter genre: ')
    # if user's input is the same as we have in GENRES, we move forward
        ...
        for genre in GENRES.keys():
            if enter_genre_input in genre:
                ...
                while enter_input_movie not in GENRES.get(genre):
                    print('Movie', enter_input_movie, 'not found. Please try again.')
                    enter_input_movie = input('Enter movie: ')
                else:
...
if user_input_genre == 'n':
    ...
    if user_input_actor == 'y':
        ...
        for actor in CAST.values():
            for x in actor:
                if x not in available_actors:
                    available_actors.append(x)
        print('Available Actors:', available_actors)

    enter_actor_input = input('Enter actor: ')
    # pretty the same list if available actors with which we compare user's actor input
    more_available_actors = []
    for actor in CAST.values():
        for x in actor:
            if x not in more_available_actors:
                more_available_actors.append(x)
    # when user's actor input doesn't match any of actors in CAST, it goes back and asks to provide an actor again
    while enter_actor_input not in more_available_actors:
        print('Actor', enter_actor_input, 'not found. Please try again.')
        enter_actor_input = input('Enter actor: ')
    else:
        ...
        while enter_movie_input not in available_movies:
            print('Movie', enter_movie_input, 'not found. Please try again.')
            enter_movie_input = input('Enter movie: ')
